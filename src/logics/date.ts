...
export function formatToDollars(value: string | number, showSymbol: boolean = true): string {
  if (typeof value === 'string') {
      const numericValue = parseFloat(value.replace(/[^0-9.-]+/g, ""));
      if (isNaN(numericValue)) {
          return '$0.00';
      }
      return showSymbol ? numericValue.toLocaleString('en-US', { style: 'currency', currency: 'USD', currencyDisplay: 'symbol' }) : numericValue.toLocaleString();
  } else if (typeof value === 'number') {
      return showSymbol ? value.toLocaleString('en-US', { style: 'currency', currency: 'USD', currencyDisplay: 'symbol' }) : value.toLocaleString();
  } else {
      return '$0.00';
  }
}
...
export function sortBy(objects:any,property:string) {
  // Sort the objects based on the updatedAt and addedAt properties
  objects.sort((a:any, b:any) => {
      // Convert updatedAt strings to Date objects for comparison
